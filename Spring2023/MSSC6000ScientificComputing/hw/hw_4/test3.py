...
class Schedule:
    ...
    def upper_bound(self):
        ...
        remaining_jobs = [self.all_jobs[i] for i in range(0,len(self.all_jobs)) if i not in self.jobs_done]
        remaining_jobs = [remaining_jobs[i] for i in range(0, len(remaining_jobs)) if remaining_jobs[i].deadline >= self.time_used]
		
		## REMOVE JOBS THAT CANNOT BE COMPLETED ANYMORE
        ...
        time = self.time_used
        max_duration = max(job.deadline for job in remaining_jobs) - time #AKA capacity
        total_value = 0
