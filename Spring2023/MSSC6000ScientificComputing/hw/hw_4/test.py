...
class Schedule:
	...
	def upper_bound(self):
		...
		time = self.time_used
		profit = sum(job.profit for job in self.all_jobs if job.index in self.jobs_done)

		# This gives a high LOWER bound
		for job in remaining_jobs:
			if time + job.duration <= job.deadline*2: 
				#Let's the deadlines can be extended, MAYBE??? 10% of the size of the jobs?
				time += job.duration
				profit += job.profit
			else:
                # If a job cannot be done before its deadline, stop adding jobs and return the current profit
				break
