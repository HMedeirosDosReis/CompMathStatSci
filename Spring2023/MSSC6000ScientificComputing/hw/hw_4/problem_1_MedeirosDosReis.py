...
# c - Upper bound for job scheduling problem  
...
class Schedule:
	...
	def upper_bound(self):
		"""
		Returns an upper bound for a partial solution
		"""
		
		# Let's relax the problem and try to come up with an upper bound that
		# does not involve the deadline, so that it is like a knapsack problem

		# Find the jobs that we didnt do, and remove the jobs that are past the 
		# deadline
		remaining_jobs = [self.all_jobs[i] for i in range(0,len(self.all_jobs)) 
		    if i not in self.jobs_done]
		remaining_jobs = [remaining_jobs[i] for i in range(0, len(remaining_jobs)) 
		    if remaining_jobs[i].deadline >= self.time_used]

		# Sort the job by profit/duration
		...
		time = self.time_used
		# AKA capacity in knapsack
		max_duration = max(job.deadline for job in remaining_jobs) - time 
		total_value = 0
		# Solve the fractional knapsack, which has an optimal solution 
		for job in remaining_jobs:
			# Now, let's consider the deadline, if I can complete this job 
			# before the deadline, keep going
			if job.deadline < time +job.duration: 
				continue
			elif job.duration <= max_duration:
				if job.deadline < time +job.duration: 
					time +=job.duration
...
# Run time between 101 and 112 seconds with n=50
