...
X, Y = np.meshgrid(x, y)
Z = (np.abs(X)+np.abs(Y))*np.exp(-np.sin(X**2)-np.sin(Y**2))

plotter = ContinuousOptimizationPlotter([X, Y, Z], maximization=False)

eval_function = lambda x,y: (abs(x)+abs(y))*math.exp(-math.sin(x**2)-math.sin(y**2))
bounds = [[-5, 5]]*2
tweak_delta = 0.1
cns_func = ContinuousFunction(eval_function, bounds, tweak_delta)

PSO = PSOManager(cns_func, 2, 10, alpha=0.9, max_speed=0.5, maximization=False)
gen = 0
