...
X, Y = np.meshgrid(x, y)
Z = -np.sin(X)*np.sin(X**2/math.pi)**20 + np.sin(Y)*np.sin(2*Y**2/math.pi)**20

plotter = ContinuousOptimizationPlotter([X, Y, Z], maximization=True)
...
PSO = PSOManager(cns_func, 2, 100, alpha=0.9, beta=1, gamma=1, max_speed=1, maximization=True)
gen = 0
