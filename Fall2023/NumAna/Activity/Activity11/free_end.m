...
% u'_opt
ydotopt = fzero(@F, [-1 1]) ;                %Value of y' that will hit the BC

%Solve the IVP using ydotopt
% u'
ydot = @(t,y) [y(2);
    -w*(Tair - y(1))-sigma*(Tair.^4-y(1).^4)];      %Set up the system of ODEs
tspan = [0 L]; ICs = [ydotopt yL];          %Set the BCs
...
ydot = @(t,y) [y(2);
    -w*(Tair - y(1))-sigma*(Tair.^4-y(1).^4)]; %Define the system of ODEs
tspan = [0 L]; ICs = [s yL];           %Set the BCs
